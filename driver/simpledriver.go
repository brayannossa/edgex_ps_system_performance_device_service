...
func (s *SimpleDriver) HandleReadCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []sdkModels.CommandRequest) (res []*sdkModels.CommandValue, err error) {
	...
	if len(reqs) == 1 {
		...
		if reqs[0].DeviceResourceName == "RAM" {
			RAM, err := CheckRAM()
			if err != nil {
				return nil, err
			}
			if err == nil {
				cv, _ := sdkModels.NewCommandValue(reqs[0].DeviceResourceName, common.ValueTypeFloat64, RAM)
				res[0] = cv
			} else {
				return nil, err
			}
		...
		if reqs[0].DeviceResourceName == "InternetSpeed" {
			...
			if err == nil {
				cv, _ := sdkModels.NewCommandValue(reqs[0].DeviceResourceName, common.ValueTypeFloat64, downloadSpeed)
				res[0] = cv
			} else {
				return nil, err
			}
